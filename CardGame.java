...
public class CardGame {
    ...
    public void has(int player, int type, int no, int[][][] mem) {
    ...
    public boolean[] remove(int[] inp, int[][][] mem) {
        ...
        for (int i = 0; i < cardPerPlayer; i++) { // There will be 13 onePlays
            ...
            for (int j = 0; j < players; j++) {
                if (curplayer == myID) {
                    ...
                    int[] r = afterOnePlay(myID, new int[players][2], playerCard.clone(), l, -1);
                    ...
                    // int[] m = solver(myID, myID, new int[players][2], playerCard, i);
                    // String typeee = (m[0] == 0 ? "H" : m[0] == 1 ? "P" : m[0] == 2 ? "E" : "C");
                    printList(l);
                    // System.out.println("Suggestion: Play " + typeee + " " + m[1] + " " + m[2] +
                    // ", Enter your move: ");
                    System.out.println("Enter your move: ");
                    inp = get();
                    onePla[curplayer][0] = inp[0];
                    onePla[curplayer][1] = inp[1];
                    remove(inp, playerCard);
                } else {
    ...
    public int[] afterOnePlay(int curplayer, int[][] inp, int[][][] mem, ArrayList<int[]> l, int startCardType) {
        // l will store every card played by me and its point
        int[][] np = inp.clone();
        int[][][] m = mem.clone();
        int[] max = new int[3];
        max[2] = -1;
        ...
        for (int i = 0; i < 4; i++) {
            for (int j = 2; j < cardPerPlayer + 2; j++) {
                if ((m[curplayer][i][j] == 1 || m[curplayer][i][j] == 2)
                    ...
                    np[curplayer][0] = i;
                    np[curplayer][1] = j;
                    boolean[] b = remove(new int[] { i, j }, m);
                    if (curplayer == myID) {
                        t = afterOnePlay((curplayer + 1) % players, np, m, l, i);
                        max[2] = t[2];
                        max[0] = i;
                        max[1] = j;
                        l.add(max.clone());
                    } else if ((curplayer + 1) % players != myID) {
                        t = afterOnePlay((curplayer + 1) % players, np, m, l, startCardType);
                        if (t[2] > max[2]) {
                            max[2] = t[2];
                            max[0] = i;
                            max[1] = j;
                        }
                        // System.out.println("Hello " + curplayer);
                    } else {

                        int[] k = onePlay(np, myID);
                        // for (int j2 = 0; j2 < players; j2++) {
                        // System.out.println("Player " + j2 + " " + np[j2][0] + " " + np[j2][1]);
                        // }
    ...
    public void undoremove(boolean[] hasBefore, int type, int num, int[][][] mem) {
        for (int i = 0; i < players; i++) {
            if (hasBefore[i] == true) {
                mem[i][type][num] = 1;
            }
    ...
    public int[] solver(int startPlayerId, int curplayer, int[][] oneP, int[][][] memory, int count) { // version 2
